...
uchar i2c_write(uchar addr, uchar reg,uchar n,uchar *tmp){
  return Multiple_writeI2C(addr<<1,tmp,reg,n);
}
uchar i2c_read(uchar addr,uchar reg,uchar n,uchar *tmp){
  return Multiple_readI2C(addr<<1,tmp,reg,n);
}

int get_ms( unsigned long* cnt ){
  return 0;
}
