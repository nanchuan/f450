...
#define Flash_w() flashd[0] = KP;flashd[1] = KPH;flashd[2] = KD;\
  flashd[3] = KPZ;flashd[4] = KIH;flashd[5] = KDZ;flashd[6] = KDH;flashd[7] = KI    //写flash
#define Flash_r() KP = flashd[0];KPH = flashd[1];KD = flashd[2];\
  KPZ = flashd[3];KIH = flashd[4];KDZ = flashd[5];KDH = flashd[6];KI = flashd[7]    //读flash
...
void main(void)
{
  ...
  int pst1=0,pst2=0;                    //pid积分
  int KP,KI,KD,KPZ,KDZ,KPH,KIH,KDH;     //PID参数 55 0 9 60 0 10 15 1 28
//  int acc2_tmp=0,shkabsprt=ACC_2;
  int throttle_out=Pwm_min,throttle=Pwm_min,pidout1=0,pidout2=0,pidout3=0;      //pid计算输出
  
  int pwm1=Pwm_min,pwm2=Pwm_min,pwm3=Pwm_min,pwm4=Pwm_min;      //pwm 电机
  
  int flashd[FLASHL];
  
  int avin,hight=0,hight_ccn=0,high_tmpE=0;
  int hight_p=0,hight_i=0,hight_d=0,hight_pp=0;//hight_cc=0,,hight_p_2=0
  int throttle_p=0,throttle_i=0,throttle_d=0,throttle_ii=Pwm_min;//,hight_i=0,throttle_tmp;
...
//          KIZ = rxbuf[15];
        ...
        case 8:
          ...
          Led4_1();
          break;
        case 9:                                                 //cahnge to auto
          Mark |= AUTO;
          throttle_ii = throttle- throttle_p- throttle_i;
          Led1_0();count++;
          break;
        case 10:                                                //change to mainul
          Mark &=~AUTO;
          if(Mark&ERR_H)
            Mark &=~ERR_H;
          Led1_0();count++;
...
//    }
    ...
    if(Mark&TA0Mark)            //定时器a0时间到达,4ms     
    {
      ...
      if(!(Mark&ERROR))
      { 
        if((abs(Roll)>500)||(abs(Pitch)>500))  
        {
          Mark |= ERROR;        //姿态角大于50度,锁定飞行器
          Led2_0(); 
        }
        else{            
          derr11 = error1;
          ...
          if(abs(error1)<30)
            if(!(((pst1>5000)&&(error1>0))||((pst1<-5000)&&(error1<0))))
              pst1 += error1;           //(abs(pst1)<5000)
          pidout1 = ((long)error1*KP+ (long)pst1*KI/10- (long)gyro[0]*KD)/100
            + (error1- derr11);         //
          
          derr21 = error2;
          error2 = point2- Roll;        //pid for Roll
          if(abs(error2)<30)
            if(!(((pst2>5000)&&(error2>0))||((pst2<-5000)&&(error2<0))))
              pst2 += error2;           //abs(pst2)<5000
          pidout2 = ((long)error2*KP+ (long)pst2*KI/10+ (long)gyro[1]*KD)/100
            + (error2- derr21);         //
          ...
          if(abs(throttle_out- throttle)<100)
    ...
    if((Mark&TA1Mark))                  //定时器a1时间到达,30ms
    {
      ...
      if(!(Mark&HIGH)){
        Mark |= HIGH;
        hight_d = 0;
        if((abs(high_tmp- hight)<100)){
//          hight_d = (hight_d*6+ (hight- high_tmp)*4)/10;//hight- high_tmp
          hight_d = hight;
          hight = (hight*(10- FLTC)+ high_tmp*FLTC)/10;  
          hight_d = hight_d- hight;
          hight_ccn = 0;
        }
        else{ 
          if(abs(high_tmpE- high_tmp)<200){
            hight_ccn++;
            if(hight_ccn>10){
//            Mark |= ERR_H;
              hight = high_tmp; 
        ...
        if((!(Mark&ERROR))&&(Mark&AUTO)){          
          if(hight<1000){
            ...
            if(abs(hight_p)<50)
            {
              if(!(((hight_i>5000)&&(hight_p>0))||((hight_i<-5000)&&(hight_p<0)))){/*(abs(hight_p)<100)*/
                hight_i += hight_p;
                throttle_i = (int)((long)hight_i*KIH/1000);
              }
            }
            else if(hight_p>0){ 
              if(hight_p>=hight_pp)
          ...
          throttle = throttle_p+ throttle_i+ throttle_d+ throttle_ii;
        }
      }
      ...
      if(Mark&PIDC){
        Mark &=~PIDC;
        PID_update(txbuf,KP,KI,KD,KPZ,0,KDZ,KPH,KIH,KDH);        //发送pid参数
      }
      else{
        Scop_update(txbuf,Roll,Pitch,Yaw,hight,throttle,point2,point1,point3,
                   (pwm1-Pwm_min)/10,(pwm2-Pwm_min)/10,(pwm3-Pwm_min)/10,(pwm4-Pwm_min)/10,
                   avin,Mark,count,
                   /*Roll*/hight_d,hight_p/*-gyro[1]/10*/,hight/*-accel[0]/10*/);
      }     
      NRF_TxPacket_AP(txbuf);      //启用无线发送   
...
//          scop_quat(txbuf,quatf); 
        ANO_scopdatAE(txbuf,throttle- 500,point3+1500,1500-point2,point1+1500,hight,Mark,
